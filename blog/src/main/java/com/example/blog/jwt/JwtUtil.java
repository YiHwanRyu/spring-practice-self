...
public class JwtUtil {
    ...
    // 2. JWT Cookie에 저장
    public void addJwtToCookie(String token, HttpServletResponse res) { // HttpServletResponse: 쿠키생성시 필요
        try {
            token = URLEncoder.encode(token, "utf-8").replaceAll("\\+", "%20"); // 쿠키에는 공백불가 -> 정규 표현식으로 제거 후 utf-8인코딩

            Cookie cookie = new Cookie(AUTHORIZATION_HEADER, token); // NAME-Value 구조
            cookie.setPath("/");

            //Response 객체에 Cookie 추가(헤더에 담김)
